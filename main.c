...
#define DELAY 30000 // Réduction du délai pour plus de fluidité
...
int main() {
    ...
    for (int i = 0; i < ENEMY_ROWS; i++) {
        for (int j = 0; j < ENEMY_COLS; j++) {
            enemies[i][j].x = j * 6 + 2; // Plus d'espace horizontal
            enemies[i][j].y = i * 3 + 2; // Plus d'espace vertical
            enemies[i][j].alive = 1;
        }
    }
    ...
    while (!game_over) {
        ...
        ch = getch();
        if (ch == KEY_LEFT && player_x > 1) { // Limite gauche
            player_x -= 2; // Déplacement plus fluide
        } else if (ch == KEY_RIGHT && player_x < max_x - 5) { // Limite droite
            player_x += 2;
        } else if (ch == ' ') { // Tirer une balle
            for (int i = 0; i < MAX_BULLETS; i++) {
                if (!bullets[i].active) {
                    bullets[i].x = player_x + 1; // Centrer la balle
...
void draw_player(int x, int y) {
    attron(COLOR_PAIR(1));
    mvprintw(y, x, "/^\\"); // Joueur plus large
...
void move_enemies(Enemy enemies[][ENEMY_COLS], int rows, int cols, int *direction, int *game_over) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            if (enemies[i][j].alive) {
                if ((enemies[i][j].x <= 0 && *direction == -1) || (enemies[i][j].x >= COLS - 5 && *direction == 1)) {
                    *direction *= -1;
                    shift_down = 1;
                }
            }
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            if (enemies[i][j].alive) {
                enemies[i][j].x += *direction * 2; // Déplacement plus fluide
                if (shift_down) {
                    enemies[i][j].y++;
                    if (enemies[i][j].y >= LINES - 2) {
                        *game_over = 1;
                    }
                }
            }
        ...
        if (bullets[i].active) {
            for (int j = 0; j < rows; j++) {
                for (int k = 0; k < cols; k++) {
                    if (enemies[j][k].alive && bullets[i].x >= enemies[j][k].x &&
                        bullets[i].x <= enemies[j][k].x + 2 && bullets[i].y == enemies[j][k].y) {
                        enemies[j][k].alive = 0;
                        bullets[i].active = 0;
                        (*score)++;
                        break;
